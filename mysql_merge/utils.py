...
def map_fks(db_map, force_input=True):
  pks = dict((table_name, table_data['primary'].keys()[0]) for table_name, table_data in db_map.items() if len(table_data['primary'].keys()) > 0)
  tables = pks.keys()
  ignores = {}
  ...
  if force_input:
    ...
    for table_name, table_data in db_map.items():
      for column_name, column_data in db_map[table_name]['fk_create'].items():
        print "  '%s.%s': '%s.%s', " % (table_name, column_name, column_data['parent'], column_data['parent_col'])
